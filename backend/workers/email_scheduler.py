    ...
    async with _email_queue_lock:
            ...
            item = await db.email_queue.find_one_and_update(
                {
                    ...
                    "$or": [
                        {"retry_count": {"$lt": 3}},
                        {"kind": {"$in": ["welcome", "affiliate_welcome", "affiliate_nudge", "checkout_recovery"]}, "retry_count": {"$lt": 10}}
                    ]
                },
                {"$set": {"status": "sending"}},
                sort=[("scheduled_at", 1)],
        ...
        for item in pending:
            try:
                ...
                elif kind == "checkout_recovery":
                    success, error_msg = await send_checkout_recovery_email(
                        name=item["name"],
                        email=item["email"],
                    )
                else:
                ...
                if success:
                    ...
                    if kind not in ("welcome", "affiliate_welcome", "affiliate_nudge", "checkout_recovery"):
                    ...
                    max_retries = 10 if item.get("kind") in ("welcome", "affiliate_welcome", "affiliate_nudge", "checkout_recovery") else 3
